...
class TrafficDetourHelper {
	...
	private static final String TRACE_TAG = "CD_DETOUR";
	...
	/**
	 * One line per decision, into the drive log rather than only logcat.
	 *
	 * <p>This feature CHANGES THE ROUTE. Every branch that declines - no spans matched, no drivable
	 * alternative, no budget to score with, not worth the switch - has to be visible afterwards, or
	 * a drive where nothing happened is indistinguishable from a drive where it was broken.
	 */
	private static void cdLog(String message) {
	...
	private static void evaluate(OsmandApplication app, RoutingHelper helper, ApplicationMode mode,
		...
		if (jamRoadIds.isEmpty()) {
			cdLog("delay +" + delaySeconds + " s but no jam spans matched our roads - skip");
			return;
		}
		...
		if (detour == null || !detour.isCalculated()) {
			cdLog("no drivable route around the jam (" + jamRoadIds.size() + " segment(s)) - staying");
			return;
		}
		...
		if (!GoogleTrafficHelper.claimDetourDelayPoll(app)) {
			cdLog("daily delay budget spent - cannot score the detour, staying");
			return;
		}
		...
		cdLog("current ~" + (currentLiveS / 60) + " min live vs detour ~"
